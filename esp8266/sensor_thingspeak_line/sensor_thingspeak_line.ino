...
#define LINE_TOKEN_TMP "ZrHx4oHAJMZyYuy9HJb3kxgsjXcB5ekgjdzXJsF0V61"      // Token สำหรับแจ้งเตียนไปยัง LINE

#define API_KEY "5FXVUTMEBRIKVPR1"                                        // API Key สำหรับใช้งาน thingspeak
...
void loop() {
    ...
    if ((ts - ts2 >= 120000) && (WiFi.status() == WL_CONNECTED)) {
      read_sensor();
    }
    delay(10);
...
void read_sensor() {
    ...
    sendToThingSpeak (t, h, SoilMoisture, LDR);
...
void sendToThingSpeak (int t, int h, int SoilMoisture, int LDR) {
    http.begin("http://api.thingspeak.com/update?api_key=" + API_KEY + "&field1=" + String(t) 
